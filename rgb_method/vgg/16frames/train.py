...
def train_model(model, n_epoch, optimizer, scheduler, train_loader, val_loader, model_dir):
    ...
    for epoch in range(n_epoch):
        ...
        for idx, (buf, labels) in enumerate(train_loader):
            ...
            if (idx + 1) % interval == 0:
                print('[train-{}/{}] [{}/{}] [acc-{:.4f} loss-{:.4f}]'.format(epoch, n_epoch, corrects, total,
                                                                              corrects / total, total_loss / total))

            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

        print('[train-{}/{}] [{}/{}] [acc-{:.4f} loss-{:.4f}]'.format(epoch, n_epoch, corrects, total, corrects / total,
                                                                      total_loss / total))
        with open('./{}.txt'.format(os.path.basename(__file__).split('.')[0]), 'a+') as record:
            record.write('[train-{}/{}] [{}/{}] [acc-{:.4f} loss-{:.4f}]'.format(epoch, n_epoch, corrects, total,
                                                                                 corrects / total, total_loss / total))
        ...
        with torch.no_grad():
            ...
            scheduler.step(loss)
            print(
                '[val-{}/{}] [{}/{}] [acc-{:.4f} loss-{:.4f}]'.format(epoch, n_epoch, corrects, total, corrects / total,
                                                                      total_loss / total))
            ...
            if acc >= 0.50:
                try:
                    ...
                    torch.save(model.state_dict(),
                               os.path.join(model_dir, 'vgg16_16_{:.4f}.pth'.format(corrects / total)))
